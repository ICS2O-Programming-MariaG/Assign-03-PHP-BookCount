...
<?php
  ...
  if ($pageSize == 1) {
    ...
    //Second if/then statement: if fontSize is 11
    else if ($fontSize == 11) {
      $pageCount = number_format($wordCount / 350, 2);
      echo "The book has approximately $pageCount pages filled with text.";
    }
    //Third if/then statement: if fontSize is 12 (both other situations are false)
    else {
  ...
  //Second deciding option: if page size is 6 x 9 (option value is 2)
    ...
    //fontSize is 12 (both other situations are false)
    else {
